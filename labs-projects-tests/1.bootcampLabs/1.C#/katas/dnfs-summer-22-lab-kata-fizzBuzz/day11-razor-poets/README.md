...
Use the slides of today, solve the kata below, twice:

1. Create a solution in a folder called `FizzBuzz.xUnit`
   1. Create an xUnit project `FizzBuzz.xUnit.Tests`
   1. Add the `FluentAssertion` package to `FizzBuzz.xUnit.Tests`
   1. Create a class library project `FizzBuzz.xUnit`
   1. Set a reference from the test project to the class library
   1. Run the `dotnet test` command to run the initial tests
   1. Keep running the tests using the watch command.
   1. Solve the problem by writing the test first
1. Create a solution in a folder called `FizzBzz.NUnit`
   1. Do the same things as above, but use `NUnit` instead of `xUnit`
   1. NUnit documentation found here <https://docs.nunit.org/>, and a tutorial found here <https://docs.microsoft.com/en-us/dotnet/core/testing/unit-testing-with-nunit>
