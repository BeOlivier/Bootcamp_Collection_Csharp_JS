...
namespace Salt.Stars.Web.Tests
{
  public class HeroesPageTests
  {
    ...
    public HeroesPageTests()
    {
      ...
      _apiMockClient.Setup(apiClient => apiClient.GetHeroes().Result).Returns(MOCK_RESPONSE);

      pageUnderTest = new HeroesPage(_apiMockClient.Object);
    }
    ...
    async public void should_get_heroes()
    {
      ...
      // Assert
      _apiMockClient.Verify(p => p.GetHeroes());
