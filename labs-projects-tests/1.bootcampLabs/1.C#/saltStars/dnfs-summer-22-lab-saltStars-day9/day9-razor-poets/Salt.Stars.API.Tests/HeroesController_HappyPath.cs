...
namespace Salt.Stars.API.Tests
{
  ...
  public class HeroesController_HappyPath
  {
    ...
    Hero MOCK_HERO_DATA = new Hero
    {
      Name = "Captain Kirk",
      Url = "https://trekkiapi.dev/api/people/3232/",
      HomeWorldUrl = "https://trekkiapi.dev/api/planets/2323/"
    };
    Planet MOCK_PLANET_DATA = new Planet { Name = "Earth" };
    ...
    public HeroesController_HappyPath()
    {
      ...
      _apiMockClient.Setup(apiClient => apiClient.getHerosFromSwapi().Result).Returns(MOCK_HEROES_DATA);
      _apiMockClient.Setup(apiClient => apiClient.getHeroFromSwapi(It.IsAny<int>()).Result).Returns(MOCK_HERO_DATA);
      ...
      _starFileMockClient.Setup(fileClient => fileClient.GetStarsForHero(It.IsAny<int>()).Result).Returns(MOCK_NUMBER_STARS);

      this.controller = new HeroesController(_apiMockClient.Object, _starFileMockClient.Object);
    }
    ...
    [Fact]
    public void get_hero_should_return_starRating_mocked_data()
    {
      // Act
      var response = controller.GetHeroAsync(234);
    ...
    [Fact]
    public void get_hero_should_return_planet_mocked_data()
    {
      // Act
      var response = controller.GetHeroAsync(234);
      ...
      // Assert
      var hero = response.Result.Value.Hero;
      hero.HomeWorldUrl.Should().Be(MOCK_HERO_DATA.HomeWorldUrl);
      hero.HomeWorldName.Should().Be(MOCK_PLANET_DATA.Name);
    }
  }
}
