...
namespace Salt.Stars.API.Tests
{
  ...
  public class HeroesController_SadPath
  {
    ...
    public HeroesController_SadPath()
    {
      ...
      _apiMockClient.Setup(apiClient => apiClient.getHerosFromSwapi().Result).Throws<Exception>();
      _apiMockClient.Setup(apiClient => apiClient.getHeroFromSwapi(It.IsAny<short>()).Result).Throws<Exception>();

      this.controller = new HeroesController(_apiMockClient.Object, null);
    }
