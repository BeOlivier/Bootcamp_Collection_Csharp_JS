...
namespace Salt.Stars.API.Tests
{
  ...
  public class HeroesController_HappyPath
  {
    ...
    public HeroesController_HappyPath()
    {
      ...
      _apiServiceMock.Setup(apiService => apiService.getHerosFromSwapi().Result).Returns(MOCK_HEROES_DATA);
      _apiServiceMock.Setup(apiService => apiService.getHeroFromSwapi(It.IsAny<short>()).Result).Returns(MOCK_HERO_DATA);
