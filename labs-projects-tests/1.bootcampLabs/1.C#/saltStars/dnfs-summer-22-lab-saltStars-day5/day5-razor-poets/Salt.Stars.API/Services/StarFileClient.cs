...
namespace Salt.Stars.API.Services
{
  public class StarFileClient : IStarFileClient
  {
    ...
    {
      var lineArray = await getFileContent();
      
      if (lineArray != null)
      {
        for(int i = 0; i < lineArray.Length; i++)
        {
          var lineData = getLineData(lineArray[i]);
          if (getHeroId(lineArray[i]) == heroId)
          {
            lineArray[i] = heroId + ", " + stars;
            await writeLinesToFile(lineArray);
      ...
      await writeLinesToFile(new string[] {heroId + ", " + stars});

      return await GetStarsForHero(heroId); 
    }
