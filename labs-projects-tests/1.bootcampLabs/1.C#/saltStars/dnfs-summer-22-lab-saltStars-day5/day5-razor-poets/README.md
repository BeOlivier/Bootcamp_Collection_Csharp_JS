...
- Call the API from the Salt.Stars.Web application
...
- Show the stars on the Razor page

  - Update `HeroResponse.Hero` to hold a `StarRating` property
  - Show the `Model.HeroResponse.Hero.StarRating` property in a suitable/good looking way on the page

- Finally - allow the user to update the star counts

  - Add form `<form method="post">` around to include the `Model.HeroResponse.Hero.StarRating`
  - Implement the `HeroPage.OnPostAsync` function to call the `IHeroAPIClient.UpdateStars` method
  - When it succeed do a `RedirectToAction("OnGetAsync", new { id = id });` to refetch the result, which is the old-school way of ensuring that the data is updated.
