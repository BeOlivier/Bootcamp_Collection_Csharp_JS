...
namespace Salt.Stars.API.Services
{
  public class SwApiClient : ISwApiClient
  {
    ...
    public async Task<Planet> getPlanetFromSwapi(int id)
    {
      ...
      var url = $"https://swapi.py4e.com/api/planets/{id}";
      var planetTask = client.GetStreamAsync(url);

      return await JsonSerializer.DeserializeAsync<Planet>(await planetTask);
    }
    public async Task<PlanetListResponse> getPlanetsFromSwapi()
    {
      var client = new HttpClient();
      client.DefaultRequestHeaders.Accept.Clear();
      client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
      ...
      var planetsTask = client.GetStreamAsync(url);

      return await JsonSerializer.DeserializeAsync<PlanetListResponse>(await planetsTask);
    }
