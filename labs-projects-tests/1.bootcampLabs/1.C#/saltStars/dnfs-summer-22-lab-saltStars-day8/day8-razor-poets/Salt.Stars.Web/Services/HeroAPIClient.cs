...
namespace Salt.Stars.Web.Services
{
  public class HeroAPIClient : IHeroAPIClient
  {
    ...
    public async Task<PlanetResponse> getPlanetFromSwapi(int id)
    {
      // var client = new HttpClient();
      // client.DefaultRequestHeaders.Accept.Clear();
      // client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
      ...
      var planeetTask = client.GetStreamAsync(url);
      // var jsonTask = client.GetStringAsync(url);
      // Console.WriteLine(await jsonTask);

      return await JsonSerializer.DeserializeAsync<PlanetResponse>(await planeetTask);
    }
  }
}
